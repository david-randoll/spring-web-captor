...
@TestPropertySource(properties = {
        "web-captor.excluded-endpoints[0].path=/test/filter/default-method"
})
class DefaultMethodIsAllTest {
    @Autowired
    ...
    @Test
    void allMethodsShouldBeExcludedByDefault() throws Exception {
        mockMvc.perform(get("/test/filter/default-method"))
                .andExpect(status().isOk());

        mockMvc.perform(post("/test/filter/default-method"))
                .andExpect(status().isOk());
