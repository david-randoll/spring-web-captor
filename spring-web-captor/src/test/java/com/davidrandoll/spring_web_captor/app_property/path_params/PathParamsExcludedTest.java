...
class PathParamsExcludedTest {
    ...
    @Test
    void testPathParamsNotCapturedWhenDisabled() throws Exception {
        mockMvc.perform(get("/test/property/path-param/456"))
                .andExpect(status().isOk());

        var request = eventCaptureListener.getRequestEvents().getFirst();
        assertNull(request.getPathParams(), "Path params should not be captured");
    }
}