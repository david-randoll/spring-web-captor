...
@TestPropertySource(properties = {
        "web-captor.excluded-endpoints[0].method=GET",
        "web-captor.excluded-endpoints[0].path=/test/excluded"
})
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class ExcludedEndpointTest {
    @Autowired
    ...
    @Test
    void requestToExcludedEndpointShouldNotPublishEvent() throws Exception {
        mockMvc.perform(get("/test/excluded"))
                .andExpect(status().isOk());

        assertTrue(eventCaptureListener.getRequestEvents().isEmpty(), "Request event should not be published");
        assertTrue(eventCaptureListener.getResponseEvents().isEmpty(), "Response event should not be published");
    }
}
